...
$digest --review h=$review_host,u=$review_user,p=$review_password,D=$review_schema,t=profiler-queries-$review_server --review-history D=$review_schema,t=profiler-histories-$review_server $slow_log/$review_server.log | mailx -s "SQL Profiler report for $review_server" $email_to
